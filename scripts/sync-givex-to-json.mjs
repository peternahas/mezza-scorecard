...
/**
  ...
  * either) and is tracked separately as CATERING_DEPARTMENTS until
  * Peter/Ashley confirm which bucket it belongs in (same open question
  * as the net vs. gross Total_Sales basis -- see SETUP.md).
  *
...
const excludedOutlets = new Map(); // outletId|storeName -> { count, reason }
let processed = 0;
  let skipped = 0;
...
for (const key of keys) {
  ...
  if (matchedBy === "excluded") {
    ...
    const existing = excludedOutlets.get(excludeKey) || { count: 0, reason: excludeReason };
    existing.count += 1;
    excludedOutlets.set(excludeKey, existing);
...
const output = {
  ...
  excluded_outlets: Object.fromEntries(
    [...excludedOutlets.entries()].map(([k, v]) => [k, v])
    ),
};
...
await mkdir("data", { recursive: true });
  ...
  if (excludedOutlets.size > 0) {
    console.log(`Note: ${excludedOutlets.size} distinct known non-store outlet(s) were excluded on purpose -- see "excluded_outlets" in the output file.`);
  }
}
